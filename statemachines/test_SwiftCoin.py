...
@pytest.fixture()
def contract2test():
    token = project.load('/home/honeybadger/2test').SwftCoin
    yield token
...
class SwftCoin(StateMachine):
    def __init__(self,accounts,contract2test):
        ...
        self.contract = contract2test.deploy(1000,"SwiftCoin",18,"SFC",{"from": accounts[0]})
...
def test_stateful(contract2test,accounts,state_machine):
    settings = {"stateful_step_count": 10, "max_examples": 40, "report_multiple_bugs":True,"derandomize":False,"database":None, "print_blob":True}
    state_machine(SwftCoin,accounts,contract2test,settings=settings)
